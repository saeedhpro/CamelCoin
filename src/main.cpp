...
#if defined(NDEBUG)
# error "Camelcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x40c2966dcf9f8c1449ecf290255fb8ed33349b6680d48f96cc547a6a12d03556");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Camelcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Camelcoin Signed Message:\n";
...
{
    // Camelcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 230000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 230000); // Camelcoin: 230k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Camelcoin: 1.5 days
static const int64 nTargetSpacing = 1.5 * 60; // Camelcoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Camelcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Camelcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa2a8f702860e93bb9f83a09013773b564bcba6bda72b88b874ae46b21ee327dc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "General Suleimani gave a right promise. He said that ISIL and Israel would be destroyed.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1517306962;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085683311;

        if (fTestNet)
        {
            block.nTime    = 1517306950;
            block.nNonce   = 385399546;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf8008f2a966927d9e35a2d4a3ac384ba9fa83f99e7564032c4366c88d1d094cb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xba, 0xd1 }; // Camelcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CamelcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CamelcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CamelcoinMiner(CWallet *pwallet)
{
    printf("CamelcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("camelcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CamelcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CamelcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CamelcoinMiner, pwallet));
}
